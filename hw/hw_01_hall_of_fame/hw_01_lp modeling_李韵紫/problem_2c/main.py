...
def main():
    ...
    productProfits, companyProductMaterials,  companyAvailableMaterials = readDataOpenpyxl()

    # Formulate the LP model.
    model = formulateModel(productProfits, companyProductMaterials,  companyAvailableMaterials)
    #saveModel(model, constant.MPS_PATH)

    # Option 2) Read data from the Excel file using pandas, then formulate the LP model.

    # Read data.
    # productProfits, companyProductMaterials,  companyAvailableMaterials = readDataPandas()
