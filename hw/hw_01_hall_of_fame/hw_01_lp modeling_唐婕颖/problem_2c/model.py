#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# @Time: 2025/10/6 22:12
# @Author: TJY

"""
Formulate and solve the WorldLight Company Light Fixture Profit Maximization Problem using Gurobi.
"""
...
def formulateModel(productProfits, resourceProductUnits , resourceAvailableUnits,product2QuantityLimit) -> grb.Model:
    """
   ...
   resourceProductUnits  : nested dict
        The units of resourse required to produce one unit of product
        - Keys: resourse names ('Metal Frame Parts' ,' Electrical Components')
        - Values: dict
            - Keys: product names (`Product 1`, `Product 2`)
            - Values: units of resource required
    ...
    for resourse, availableunits in resourceAvailableUnits.items():
        # For each resourse, the total used units <= the available units.
        lhsExpr = grb.LinExpr()
        for product, unit in resourceProductUnits[resourse].items():
            lhsExpr += unit * unitProductionDecisions[product]

        model.addConstr(lhsExpr <= availableunits, f"{resourse}")

    model.addConstr(unitProductionDecisions['Product 2'] <= product2QuantityLimit, f"Product2QuantityLimit")
